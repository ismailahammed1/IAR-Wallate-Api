...
const addMoneyWallet = async (walletId: string, amount: number) => {
  ...
  if (wallet.status !== AccountStatus.ACTIVE) {
    throw new AppError(
      ...
      "Cannot add money to a blocked or inactive wallet"
    );
  }
  ...
  if (wallet.balance === undefined) {
    throw new AppError(
      ...
      "Wallet balance is undefined"
    );
  }
  if (typeof wallet.balance !== "number") {
    throw new AppError(
      StatusCodes.INTERNAL_SERVER_ERROR,
      "Wallet balance is not a number"
    );
  }
  if (amount < 0) {
...
const withdrawMoneyWallet = async (walletId: string, amount: number) => {
  ...
  if (wallet.status !== AccountStatus.ACTIVE) {
    throw new AppError(
      StatusCodes.FORBIDDEN,
      "Cannot withdraw money from a blocked or inactive wallet"
    );
  }
  if (wallet.balance === undefined) {
    throw new AppError(
...
const sendMoneyWallet = async (
  ...
  if (
    fromWallet.status !== AccountStatus.ACTIVE ||
    toWallet.status !== AccountStatus.ACTIVE
  ) {
    throw new AppError(
      StatusCodes.FORBIDDEN,
      "Cannot send money from/to a blocked or inactive wallet"
    );
  }
...
const cashInWallet = async (walletId: string, amount: number) => {
  ...
  if (wallet.status !== AccountStatus.ACTIVE) {
    throw new AppError(
      StatusCodes.FORBIDDEN,
      "Cannot cash in to a blocked or inactive wallet"
    );
  }
  if (wallet.balance === undefined) {
    throw new AppError(
...
const cashOutWallet = async (walletId: string, amount: number) => {
  ...
  if (wallet.status !== AccountStatus.ACTIVE) {
    throw new AppError(
      StatusCodes.FORBIDDEN,
      "Cannot cash out from a blocked or inactive wallet"
    );
  }
  if (wallet.balance === undefined) {
    throw new AppError(
